...
def get_posts():
    ...
    postsList = list(map(lambda x: x.to_json(), posts))
    return postsList
...
def createPost(token_enregistre):
    ...
    newPost = Post(
        ...
        author = Profile.query.filter_by(discord_user_id=token_enregistre.discord_user_id).first(),
...
@app.route("/get_posts_by_user_id", methods=["GET"])
def get_posts_by_user_id():

    parametres_url = request.args

    user_id = parametres_url['user_id']

    user_posts = Post.query.filter_by(author_id=user_id).order_by(desc(Post.timestamp)).limit(30).all()
    
    serialized_posts = [post.to_json() for post in user_posts]  # Sérialiser les posts en JSON
