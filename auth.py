...
class CSRFToken(db.Model):
    ...
    def to_json(self):
        return {"id": self.id, "discord_user_id": self.discord_user_id, "csrf_token": self.csrf_token}
...
def csrf_auth_required(f):
    ...
    def decorateur(*args, **kwargs):
        ...
        # #pas de Token
        # if csrf_token == None : 
        #     redirectResponse = make_response()
        #     redirectResponse.headers['Location'] = settings.FRONTEND_BASE_ROUTE + "/login"
